...
- 支持 JPEG、PNG、WebP 输入，并自动展示识别到的图片类型。
- 输出格式可自定义为 JPEG、PNG 或 WebP。
- 处理方式支持高质量、均衡和高性能，分别对应更高、更均衡、更激进的重新编码质量。
- 调整尺寸模式支持按像素自定义宽高，以及按 75%、50%、30%、15% 等比例缩小宽高。
- 调整文件大小模式支持 75%、50%、30%、15% 和极限压缩；极限压缩不受处理方式约束，会使用质量 0.2，并把长边压缩到不超过 640px 且不超过原图长边 25%。
- 选择百分比或输入像素尺寸后，会即时预览预计输出尺寸和输出 MIME 类型。
- 页面展示压缩前后预览、下载链接和清除结果按钮。
